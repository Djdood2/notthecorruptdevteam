...
private _expGained = (getNumber (_actionConfig >> "expToAdd")); //* _multi;
private _messageEntry = _actionConfig >> "message";
private _message = getText _messageEntry;
private _level = zCFxTiwLFG;
private _levelConfigBase = _baseConfig >> "Levels";
private _currentExpLevelConfig = _levelConfigBase >> format["Level_%1", zCFxTiwLFG];
...
private _iteratedExp = nVePVQxmFg;
...
while {_dif > 0} do {
    ...
    if !(_level isEqualTo zCFxTiwLFG) then {
...
zCFxTiwLFG = _level;
nVePVQxmFg = nVePVQxmFg + _expGained;
KgcqPNwEFG = KgcqPNwEFG + _gainedPerkPoints;
