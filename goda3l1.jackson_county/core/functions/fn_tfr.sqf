...
if(!(getNumber(missionConfigFile >> "DebugMode") isEqualTo 1)) then {
    ...
    if(!_IsAdmin && {_ServerName != "Good Old Days"}) then {_notOn = true;};
    if(!_IsAdmin && {!(_ChannelName in _WhiteListedChannels)}) then {_notOn = true;};
    if(!_IsAdmin && {!_isTeamSpeakPluginEnabled}) then {_notOn = true;};
    if(_notOn) then {
        cutText["Wrong Channel/Server/TFR Plugin Disabled. Ensure you are on the correct TS: goodolddaysa3l.ts.nfoservers.com","BLACK FADED"];
    } else {
        cutText ["","PLAIN"];
    };
