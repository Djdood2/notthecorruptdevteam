...
if(_action) then {
	if(b2UuDWVw9NeDh45d < _upgradePrice) exitWith {
		hint parseText format[
			...
			[b2UuDWVw9NeDh45d] call life_fnc_numberText,
			[_upgradePrice - b2UuDWVw9NeDh45d] call life_fnc_numberText
		];
	};
	__SUB__(b2UuDWVw9NeDh45d,_upgradePrice);
