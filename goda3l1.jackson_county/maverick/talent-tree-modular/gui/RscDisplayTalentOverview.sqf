...
try {
	...
	_updateGUI = {
		private _currentLevelConfig = missionConfigFile >> "Maverick_TTM" >> "Levels" >> format["Level_%1", zCFxTiwLFG];
		private _nextLevelConfig = missionConfigFile >> "Maverick_TTM" >> "Levels" >> format["Level_%1", zCFxTiwLFG + 1];
		...
		private _currentLevelRelativeExp = (nVePVQxmFg - _currentLevelBottomExp);
		private _currentLevelProgress = if (_currentLevelRelativeExp != 0) then {((nVePVQxmFg - _currentLevelBottomExp) / (_currentLevelTopExp - _currentLevelBottomExp))} else {0};

		// -- Display data
		_ctrlStrucTextCurrentRank ctrlSetStructuredText parseText format ["<t size='3' shadow='2' align='center'>%1</t>", zCFxTiwLFG];
		_ctrlTextRankName ctrlSetText getText (_currentLevelConfig >> "displayName");

		if (isClass _nextLevelConfig) then {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1/%2, Perk Points Available: %3", nVePVQxmFg - _currentLevelBottomExp, _currentLevelTopExp - _currentLevelBottomExp, KgcqPNwEFG];
			_ctrlProgress progressSetPosition _currentLevelProgress;
		} else {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1, Maximum Level, Perk Points Available: %2", nVePVQxmFg - _currentLevelBottomExp, KgcqPNwEFG];
	...
	switch (_mode) do
	{
		case "onLoad":
		{
			...
			[FFuVsqqZ, []] call _generateTreeView;
		...
		case "onTreeSelectionChanged":
		{
			...
			private _hasRequiredLevel = ((getNumber (_selectedPerkConfig >> "requiredLevel")) <= zCFxTiwLFG);
			private _limitedToFactions = getArray (_selectedPerkConfig >> "limitToSides");
			private _hasParents = if (_requiredPerk != "") then {[FFuVsqqZ, getText (_selectedPerkConfig >> "requiredPerk")] call mav_ttm_fnc_hasPerk;} else {true};

			FFuVsqqZ call _iterate;

			if (((count _limitedToFactions) == 0) || {(str playerSide) in _limitedToFactions}) then {
				if (_hasRequiredLevel) then {
					if (_hasParents) then {
						if (!_ownsPerk) then {
							if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > KgcqPNwEFG) then {
		...
		case "onButtonPurchaseClick":
		{
			...
			KgcqPNwEFG = KgcqPNwEFG - (getNumber (_selectedPerkConfig >> "requiredPerkPoints"));
